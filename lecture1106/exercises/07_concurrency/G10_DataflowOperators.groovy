...
def downloader = operator(inputs: [urls], outputs: [pagesForJava, pagesForScala]) {
...
def javaScanner = operator(inputs: [pagesForJava], outputs: [resultsFromJava]) {
...
def reporter = operator(inputs: [resultsFromJava, resultsFromScala], outputs: [reports]) {g, s ->
    assert g.url == s.url
